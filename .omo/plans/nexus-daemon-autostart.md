...
- nxc が `<repo>/.nexus/nxc-daemon.json`（`{ port, pid, startedAt }`、git ignore 済み F12）を読み書き。
...
nxc 起動 (useContext=true かつ auto-start 有効時)
  ...
  2. <repo>/.nexus/nxc-daemon.json を読む
       ...
       │         ※ 判定ロジック: 任意の HTTP 応答（400 含む）= 生存、TCP 接続拒否/タイムアウト = 死亡
       │         ├─ 生存 → そのポートで接続して終了(=利用)
       │         └─ 死亡 → 状態ファイル削除して 3 へ
       └─ なし: 3 へ
...
2. **純粋ロジック: 状態ファイル schema & パーサ** (`src/daemon-state.ts`)
   - `{ port:number, pid:number, startedAt:string }` の zod schema（`schemas.ts` に追加）。
     `startedAt` は ISO 8601 文字列で、デバッグ時のプロセス識別および将来の TTL ベースライフサイクル管理のために記録する。
...
5a. **I/O: `NEXUS_LOG_FILE` 対応** (`src/nexus-spawn.ts` の spawn 箇所)
   - 既定は `stdio: "ignore"`。`NEXUS_LOG_FILE` 環境変数が設定されている場合のみ、
     指定パスのファイルを `fs.openSync(path, "a")` で追記オープンし、
     `stdio: ["ignore", fd, fd]` として子プロセスの stdout/stderr をリダイレクトする。
   - ファイルオープン失敗（権限エラー等）時は warn ログを出力して `stdio: "ignore"` にフォールバックし、
     自動起動フロー自体は継続する（ログ取得失敗で起動を止めない）。
   - TDD 項目:
     - `NEXUS_LOG_FILE` 未設定時: ファイルが作成されないこと。
     - `NEXUS_LOG_FILE` 設定時: 指定ファイルが生成され、子プロセスの出力が書き込まれること。
     - ファイルオープン失敗時: warn が出力され `stdio:"ignore"` でフォールバックされること。
...
9. daemon 手動停止手順が README または本計画書（U4）に記載されており、`ps` / `kill` コマンドで回収可能であることをドキュメントで確認できる。
