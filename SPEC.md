...
- **プロジェクトルートの指定**: Nexus 起動時の `--project-root` 引数には、必ず対象の git トップレベルディレクトリ（`git rev-parse --show-toplevel` 相当）を解決して渡す。
...
- **停止**: `nxc` は daemon を停止しない。手動停止する場合は `.nexus/nxc-daemon.json` の `pid` を `kill` する（例: `kill $(node -p "require('./.nexus/nxc-daemon.json').pid")`）。将来的には `nxc --stop-nexus` フラグでこの操作をカプセル化する予定。

### 6.3 ローカル LLM API（OpenAI 互換）
