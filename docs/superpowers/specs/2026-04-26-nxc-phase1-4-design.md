...
/** Phase 4 の `customSuffix` (`.github/nxc.prompt.md` 由来) に課す固定トークン上限。
 *  diff/context 用の `maxTokens` 予算とは独立した system プロンプト側のキャップ。
 *  典型的なプロジェクトルール記述は数百トークン以内に収まるため、
 *  日本語で約 700–1000 文字、英語で約 3000–4000 文字に相当する 1000 を採用。 */
...
| `customSuffix` が `PROMPT_SUFFIX_MAX_TOKENS` を超える | `truncateToTokens` で末尾切り詰め後、warning ログを 1 回出力（§7.4 参照） |
...
| トークン上限 | `PROMPT_SUFFIX_MAX_TOKENS = 1000`（§6.1 で定義）。超過時は末尾切り詰め + warning ログ |
| 切り詰め方針 | token 単位で末尾を切る（先頭ルールを優先保持。意味境界判定は行わない） |
| 予算との関係 | `NEXUS_COMMIT_MAX_TOKENS`（diff/context 予算）とは独立した system プロンプト側のキャップ |
...
```typescript
import { countTokens, truncateToTokens, PROMPT_SUFFIX_MAX_TOKENS } from './tokenizer.js';
...
try {
  ...
  if (content !== null) {
    ...
    if (tokens > PROMPT_SUFFIX_MAX_TOKENS) {
      ...
      customSuffix = truncateToTokens(content, PROMPT_SUFFIX_MAX_TOKENS);
    } else {
      customSuffix = content;
    }
  }
} catch (err) {
...
切り詰めは **末尾から** 行う（先頭に書かれた優先度の高いルールを保持するため）。意味境界（パラグラフ・セクション）での切り分けは行わない —— これは複雑度に見合うメリットが薄く、ユーザーが上限内に収めれば回避できる事象であるため。
...
| `customSuffix` トークン超過 | `PROMPT_SUFFIX_MAX_TOKENS` 超過 | warning ログ + 末尾切り詰めで続行 | - |
...
| `tests/bin/nxc.test.ts` (suffix 切り詰め) | 統合 | 新規ケース | `customSuffix` トークン超過時の警告 + 末尾切り詰め |
...
| **#C** | Phase 3 | `tokenizer.ts` 新規（`PROMPT_SUFFIX_MAX_TOKENS` 含む）/ `truncate.ts` 改修 / `config.ts` の `maxTokens` 化 / README/SPEC 環境変数表更新 / `package.json` v0.2.0 bump | token ベース truncate テスト |
| **#D** | Phase 4 | `prompt-file.ts` 新規 / `prompt.ts` `customSuffix` / `bin/nxc.ts` 注入（`PROMPT_SUFFIX_MAX_TOKENS` で末尾切り詰め）/ `doctor` チェック#6 統合 / README に外部化手順追記 | `prompt-file.test.ts` 追加 |

各 PR は **独立してマージ可能**。Phase 1 → 2 → 3 → 4 の順序を推奨（Phase 2 は Phase 1 の zod スキーマを使い、Phase 4 は Phase 3 の `tokenizer` を使うため）。
...
| `customSuffix` が肥大化し LLM コンテキストを圧迫 | 応答品質低下・コンテキスト超過 | `PROMPT_SUFFIX_MAX_TOKENS = 1000` で固定上限を強制。超過時は末尾切り詰め + warning（受け入れ基準 §12-15 参照） |
...
15. `.github/nxc.prompt.md` のトークン数が `PROMPT_SUFFIX_MAX_TOKENS = 1000` を超える場合、`stderr` に超過警告が 1 回出力され、`customSuffix` は 1000 トークン以下に末尾切り詰めされた上で system プロンプトに append される（実行自体は継続）
